...
int main()
{
  ...
  uint32_t crc_1 = WLib::CRC::crc16_ccitt_false(reinterpret_cast<std::byte const*>(str_1), sizeof(str_1) - 1);
  ...
  uint32_t crc_2 =
      WLib::CRC::crc16_ccitt_false(crc_1, reinterpret_cast<std::byte const*>(str_2), sizeof(str_2) - 1);

  // check the crc with an refernce value from "http://www.sunshine2k.de/coding/javascript/crc/crc_js.html"
  if (crc_2 == 0xA401) 
    return 0;
